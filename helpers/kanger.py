...
async def Kanger(c: Client, m: Message):
    ...
    for i in range(len(Config.FORWARD_TO_CHAT_ID)):
        ...
        try:
            ...
            if ForwardToChat and ForwardToChat.chat and ForwardToChat.chat.title:
                await m.edit(text=f"Successfully Linked with `{ForwardToChat.chat.title}` !")
                await asyncio.sleep(2)
            else:
                await m.edit(text=f"Sorry, you don't have permission to send messages in that chat!")
                return 400
        except Exception as err:
    ...
    async for message in c.iter_history(chat_id=Config.FORWARD_FROM_CHAT_ID[0], reverse=True):
        ...
        try:
            ...
            print("Congratulations!\nYour Account Banned Successfully!\nI already told you to use a Fake Account. Hope you remember.")
            break
        except Exception as err:
